...
from rebus_generator.cli.loop_controller import (
    LoopRunResult,
    build_batch_command,
    build_parser,
...
class LoopControllerTests(unittest.TestCase):
    ...
    @patch("rebus_generator.cli.loop_controller.run_size")
    def test_run_cycle_continues_after_failure(self, mock_run_size):
        ...
        results = run_cycle(
            [7, 8],
            words="build/words.json",
            output_root=Path("build/batch"),
            rewrite_rounds=4,
            preparation_attempts=5,
            log_path=Path("build/loop_runner.log"),
            cwd=Path("/tmp"),
        )
    ...
    @patch("rebus_generator.cli.loop_controller.select_auto_size")
    @patch("rebus_generator.cli.loop_controller.run_size")
    ...
    @patch("rebus_generator.cli.loop_controller.subprocess.run")
