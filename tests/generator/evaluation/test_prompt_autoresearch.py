...
def _load_module():
    module_name = "rebus_generator.evaluation.campaigns.autoresearch"
...
class PromptAutoresearchTests(unittest.TestCase):
    def _tiny_prompt_tree(self, root: Path) -> Path:
        ...
        (prompts / "verify").mkdir(parents=True, exist_ok=True)
        (prompts / "verify" / "system.md").write_text("prompt\n", encoding="utf-8")
        return prompts
    ...
    def test_validate_state_detects_incumbent_mismatch(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            prompts = root / "prompts"
            (prompts / "verify").mkdir(parents=True)
            (prompts / "verify" / "system.md").write_text("same\n", encoding="utf-8")
            (paths["incumbent_prompts"] / "verify").mkdir(parents=True, exist_ok=True)
            (paths["incumbent_prompts"] / "verify" / "system.md").write_text("same\n", encoding="utf-8")
    ...
    def test_select_next_experiment_respects_v5_family_priority(self):
        mod = _load_module()
        families = mod.default_families("v5")
        state = {
            "attempted_experiments": [],
            "current_family": None,
            "experiment_set": "v5",
    ...
    def test_recover_if_interrupted_restores_incumbent_prompts(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            prompts = root / "prompts"
            (prompts / "rewrite").mkdir(parents=True)
            target = prompts / "rewrite" / "user.md"
            target.write_text("trial state\n", encoding="utf-8")

            snapshot = root / "snapshots" / "incumbent_prompts" / "rewrite"
            snapshot.mkdir(parents=True)
            (snapshot / "user.md").write_text("incumbent state\n", encoding="utf-8")
    ...
    def test_resume_existing_state_rebuilds_on_inconsistent_incumbent(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (prompts / "verify").mkdir(parents=True)
            (prompts / "verify" / "system.md").write_text("prompt\n", encoding="utf-8")
            (paths["incumbent_prompts"] / "verify").mkdir(parents=True)
            (paths["incumbent_prompts"] / "verify" / "system.md").write_text("prompt\n", encoding="utf-8")
    ...
    def test_validate_state_detects_skipped_experiment_leakage(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            state_dir = root / "state"
            paths = mod.family_paths(state_dir)
            seed = paths["seed_prompts"] / "rewrite"
            incumbent = paths["incumbent_prompts"] / "rewrite"
            prompts = root / "prompts" / "rewrite"
            seed.mkdir(parents=True, exist_ok=True)
            incumbent.mkdir(parents=True, exist_ok=True)
            prompts.mkdir(parents=True, exist_ok=True)
            base_text = (mod.runner.PROMPTS_DIR / "rewrite" / "system.md").read_text(encoding="utf-8")
            seed_text = base_text.replace(
                "- Pentru OF, păstrezi interjecția de durere ori regret și excluzi exclamația vagă de tip AH.\n",
                "",
            )
            seed_file = seed / "system.md"
            incumbent_file = incumbent / "system.md"
