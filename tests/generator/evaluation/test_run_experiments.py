...
def _load_run_experiments_module():
    module_name = "rebus_generator.evaluation.campaigns.service"
...
class RunExperimentsTests(unittest.TestCase):
    def test_build_assessment_description_includes_desc_and_file(self):
        ...
        exp = mod.Experiment(
            ...
            edits=[mod.PromptEdit(file="verify/system.md", find="a", replace="b")],
        )

        description = mod.build_assessment_description("march17/", exp)

        self.assertEqual(
            "march17/exp006 | remove technical-word hint from verify | verify/system.md",
            description,
        )
    ...
    def test_build_assessment_description_joins_multifile_paths(self):
        ...
        exp = mod.Experiment(
            ...
            edits=[
                mod.PromptEdit(file="verify/system.md", find="a", replace="b"),
                mod.PromptEdit(file="verify/user.md", find="c", replace="d"),
            ],
        )
        ...
        self.assertEqual(
            "results/exp073 | paired verify bundle OF UZ AZ | verify/system.md, verify/user.md",
            description,
        )
    ...
    def test_v4_campaign_has_8_unique_experiments(self):
        mod = _load_run_experiments_module()

        self.assertEqual(8, len(mod.V4_EXPERIMENTS))
        self.assertEqual(8, len({exp.name for exp in mod.V4_EXPERIMENTS}))
        self.assertEqual("rewrite_rule_readditions", mod.V4_EXPERIMENTS[0].family)
        self.assertEqual("rewrite_compactness_bias", mod.V4_EXPERIMENTS[-1].family)
        self.assertEqual("rewrite/system.md", mod.V4_EXPERIMENTS[0].file)
    ...
    def test_v5_campaign_has_8_unique_experiments(self):
        ...
        self.assertEqual("rewrite/system.md", mod.V5_EXPERIMENTS[0].file)
    ...
    def test_v6_campaign_has_8_unique_experiments(self):
        ...
        self.assertEqual("verify/system.md", mod.V6_EXPERIMENTS[0].file)
    ...
    def test_apply_experiment_skips_when_replacement_already_present(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (prompts_dir / "verify").mkdir(parents=True)
            target = prompts_dir / "verify" / "user.md"
    ...
    def test_best_result_summary_path_lives_under_build_state_root(self):
        ...
        path = mod.best_result_summary_path(Path("packages/rebus-generator/src/rebus_generator/prompts/production"))

        self.assertEqual(
            mod.PROJECT_ROOT / "build" / "evaluation" / "prompt_experiment_state" / "production" / "best_assessment.json",
            path,
        )
