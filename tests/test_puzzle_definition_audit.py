...
from generator.puzzle_definition_audit import run_audit
...
class PuzzleDefinitionAuditTests(unittest.TestCase):
    ...
    def test_wrapper_forwards_args_to_python_module(self):
        wrapper = Path(__file__).resolve().parent.parent / "run_puzzle_definition_audit.sh"
        with tempfile.TemporaryDirectory() as tmpdir:
        ...
        self.assertEqual(
            ["-m", "generator.puzzle_definition_audit", "--published-only", "--limit", "3"],
            captured_args,
        )
